...
module.exports = class extends Command {
    ...
    async execute(message, args) {
        ...
        switch (tagMethod) {
            ...
            default:
                return message.reply("Please use ?help tag to get help of this command")
        }
    }
    ...
    async addTag(args, message) {
        ...
        if (this.canModifyTag(message.member, null)) return message.channel.send('You can\'t create tags as you have been restricted.');
        if (!tagName) tagName = await this.awaitMessages(message, 'What do you want the tag name to be?');
        if (!tagName) return;
        if (await this.getTag(message.guild, tagName)) return message.channel.send('There is already a tag with this name.');
        ...
        if (tagContent) {
            if (tagContent.length >= 1900) return message.channel.send('You tag content can\'t be over 1900 characters.');
    ...
    async delTag(tagName, message) {
        ...
        if (tagName) {
            if (mentionedUser && message.member.hasPermission('ADMINISTRATOR')) {
                ...
                if (this.canModifyTag(message.member, tagDoc.tagCreator)) return message.channel.send('You don\'t have permissions to delete this tag.');
    ...
    async editTag(args, message) {
        ...
        if ((tagName && !tagContent) && tagHoisted === undefined) tagContent = await this.awaitMessages(message, 'What do you want to edit the tag content to?');

        const tagDoc = await this.getTag(message.guild, tagName);
        if (!tagDoc) return message.channel.send('There is not tag with this name.');
        if (this.canModifyTag(message.member, tagDoc.tagCreator)) return message.channel.send('You don\'t have permissions to edit this tag.');
        if (tagContent && tagContent.length >= 1900) return message.channel.send('You tag content can\'t be over 1900 characters.');
    ...
    async renameTag(args, message) {
        ...
        if (rename) {
            const tagDoc = await this.getTag(message.guild, tagName);
            if (!tagDoc) return message.channel.send('There is no tag with this name.');
            if (this.canModifyTag(message.member, tagDoc.tagCreator)) return message.channel.send('You don\'t have permissions to rename this tag.');
            if (await this.getTag(message.guild, rename)) return message.channel.send('There is already a tag with this name.');
    ...
    async aliasesTag(args, aliasMethod, message) {
        if (!['--add', '--del'].includes(aliasMethod)) return message.channel.send('That\'s not a valid tag alias method.');
        const tagData = this.getTagData(args);
        let tagName = tagData[0];
        let alias = tagData[1];
        if (!tagName) tagName = await this.awaitMessages(message, `What tag do you want to ${aliasMethod === '--add' ? 'add an alias too?' : 'delete an alias from?'}`);
        if (tagName && !alias) alias = await this.awaitMessages(message, `What alias do you want ${aliasMethod === '--add' ? 'add??' : 'delete?'}`);
        ...
        if (alias) {
            ...
            switch (aliasMethod) {
                case '--add':
                    if (await this.getTag(message.guild, alias)) return message.channel.send('There is already a tag with this name.');
    ...
    async infoTag(tagName, message) {
        ...
        if (tagName) {
            ...
            const embed = new MessageEmbed()
                ...
                .addField('❯ Aliases', tagDoc.tagAliases.length ? tagDoc.tagAliases.map(alias => `\`${alias}\``).join(' ') : 'None')
            ...
            if (tagDoc.lastModifiedBy && tagDoc.lastModifiedAt) {
                ...
                embed.addField('❯ Last Modified By', lastModifiedBy ? `${lastModifiedBy.tag} (ID: ${lastModifiedBy.id})` : 'Deleted User');
    ...
    async tagSearch(query, message) {
        ...
        if (query) {
            ...
            const embed = new MessageEmbed()
                .setColor('BLUE')
                .setDescription(resultTags.map(tag => `\`${tag}\``).join(' '));
    ...
    async listTags(user, message) {
        ...
        if (!allTags.length) return message.channel.send(user ? 'This member has\'t created any tags.' : 'This guild dosn\'t have any hoisted tags.');
    ...
    async getTag(guild, tagName, find = false) {
        if (find) {
            ...
            const tagDoc = await tagModel.findOne({ guildID: guild.id, tagName: tagName }).catch(console.error)
